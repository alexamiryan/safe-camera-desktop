...
public class AESCrypt {
	...
	public boolean reEncrypt(InputStream in, OutputStream out, AESCrypt secondaryCrypt) {
		return this.reEncrypt(in, out, secondaryCrypt, null, false);
	}
	public boolean reEncrypt(InputStream in, OutputStream out, AESCrypt secondaryCrypt, CryptoProgress progress) {
		return this.reEncrypt(in, out, secondaryCrypt, progress, false);
	}
	public boolean reEncrypt(InputStream in, OutputStream out, AESCrypt secondaryCrypt, CryptoProgress progress, boolean encryptionIsMy) {
		try {
			long totalRead;
			...
			if(encryptionIsMy){
				...
				in = new CipherInputStream(in, secondaryCrypt.getDecryptionCipher(decIV, decSalt));
				out = new CipherOutputStream(out, encryptionCipher);
			}
